...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        do {
            ...
            let ncMessageButtonAction = argParser.add(option: "--messagebuttonaction", kind: String.self, usage: "alert type only. The action to be performed when the message button is clicked. Either pass 'logout', 'bash' followed by commands or path to item to open on click. Can be a .app, file, URL etc. With non-.app items being opened in their default handler. Requires '--messagebutton' to be passed")
